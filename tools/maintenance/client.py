...
class WordPressClient:
    ...
    def __init__(self, username: str = None, password: str = None, base_url: str = None, domain_config=None):
        """Initialize WordPress client."""
        self.session = requests.Session()
        self.base_url = base_url or (domain_config.get('wordpress_url') if domain_config else config.get('base_url'))
        ...
        elif domain_config and 'username' in domain_config and 'password' in domain_config:
            self.authenticate(domain_config['username'], domain_config['password'])
    
    def authenticate(self, username: str = None, password: str = None) -> bool:
        """Authenticate with WordPress."""
        if auth.authenticate_from_args(username, password, self.base_url):
    ...
    def _get_api_url(self, endpoint: str = '') -> str:
        """Get full API URL for endpoint using client's base URL."""
        base = self.base_url.rstrip('/')
        api = config.get('api_endpoint', '/wp-json/wp/v2').strip('/')
    ...
    def update_post(self, post_id: int, data: Dict, force_update: bool = False) -> Dict:
        ...
        if 'featured_media' in data:
            update_data['featured_media'] = data['featured_media']
