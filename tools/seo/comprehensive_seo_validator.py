#!/usr/bin/env python3
"""
Comprehensive SEO & Site Validation Tool for SphereVista360
"""
...
class ComprehensiveSiteValidator:
    def __init__(self, config_path: str = '../../config.json'):
        ...
        # Set up auth if credentials available
    ...
    def check_seo_titles(self):
        ...
        url = f"{self.site_url}/wp-json/wp/v2/posts?per_page=20"
    ...
    def check_featured_images(self):
        ...
        url = f"{self.site_url}/wp-json/wp/v2/posts?per_page=100"
        response = requests.get(url, auth=self.auth)
    ...
    def check_categories(self):
        ...
        if response.status_code == 200:
            ...
            url = f"{self.site_url}/wp-json/wp/v2/posts?categories=1&per_page=1"
            response = requests.get(url, auth=self.auth)
            if response.status_code == 200:
    ...
    def check_internal_links(self):
        ...
        url = f"{self.site_url}/wp-json/wp/v2/posts?per_page=10"
        response = requests.get(url, auth=self.auth)
        ...
        if response.status_code == 200:
            ...
            for post in posts:
                ...
                if f'href="{self.site_url}' in content or 'href="/' in content:
                    posts_with_links += 1
    ...
    def check_robots_txt(self):
        ...
        response = requests.get(f"{self.site_url}/robots.txt")
        if response.status_code == 200:
...
def main():
    ...
    print(f"🌐 Validating: {validator.site_url}")
    print("=" * 60)
    ...
    report_file = "comprehensive_seo_report.json"
