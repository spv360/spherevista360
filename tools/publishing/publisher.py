...
class ContentPublisher:
    ...
    def _get_featured_image(self, category: str, front_matter: Dict) -> str:
        ...
        # Prefer explicit featured_image, fall back to 'image' key used in some content
        candidate = None
        if 'featured_image' in front_matter and front_matter.get('featured_image'):
            candidate = front_matter.get('featured_image')
        elif 'image' in front_matter and front_matter.get('image'):
            candidate = front_matter.get('image')

        # Validate candidate by downloading a small portion and checking dimensions
        if candidate:
            try:
                resp = requests.get(candidate, timeout=6, stream=True)
                if resp.status_code == 200:
                    # Read content up to a sensible limit (e.g., 3MB)
                    content = resp.content
                    # Use PIL to inspect image dimensions safely
                    try:
                        img = Image.open(BytesIO(content))
                        width, height = img.size
                        # Minimum acceptable dimensions (adjustable)
                        min_width, min_height = 800, 450
                        if width >= min_width and height >= min_height:
                            return candidate
                        else:
                            # Image too small: fall back to category template
                            pass
                    except Exception:
                        # If PIL can't open, fall back
                        pass
            except Exception:
                # If request fails, fall back
                pass

        # Use category template as fallback
        return self.image_templates.get((category or 'default').lower(), self.image_templates['default'])
