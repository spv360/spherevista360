...
class ContentValidator:
    ...
    def validate_seo(self, post: Dict) -> Dict:
        ...
        # Title length (10-60 characters optimal, 60 is critical SEO limit)
        if 10 <= title_length <= 60:
            score += 1
        else:
            if title_length > 60:
                issues.append(f"⚠️ CRITICAL: Title exceeds SEO limit: {title_length}/60 chars")
            else:
                issues.append(f"Title too short: {title_length}/60 chars")
