...
# Replace homepage sections with finance/investment/economy focused content
WP_SITE_URL="https://spherevista360.com"
WP_USERNAME="JK"
WP_PASSWORD=""
PAGE_ID=1686

echo "Replacing homepage sections with finance/investment/economy content"
...
# Extract the CTA and welcome sections (keep these)
CTA_SECTION=$(echo "$CURRENT_CONTENT" | sed -n '/homepage-tools-cta/,/<\/div>/p' | head -n -1)
WELCOME_SECTION=$(echo "$CURRENT_CONTENT" | sed -n '/<h2>Welcome to SphereVista360<\/h2>/,/<\/ul>/p' | head -n -1)
FINANCIAL_SECTION=$(echo "$CURRENT_CONTENT" | sed -n '/<h2>Expert Financial Analysis<\/h2>/,/<\/ul>/p' | head -n -1)

# New finance-focused content to replace the other sections
NEW_FINANCE_CONTENT='<!-- wp:group {"align":"full","style":{"spacing":{"padding":{"top":"40px","bottom":"40px"}},"color":{"background":"#f8f9fa"}},"className":"finance-spotlight"} -->
...
if echo "$RESPONSE" | grep -q '"id":'; then
  echo "Homepage updated successfully with finance-focused content!"
else
